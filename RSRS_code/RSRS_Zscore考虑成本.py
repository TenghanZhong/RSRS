...
buy_thre = 0.7
sell_thre = -0.7
transaction_cost_rate = 0.006  # 交易费率，0.6%

data_zscore = data_zscore.reset_index(drop=False)
data_zscore['flag'] = 0
data_zscore['position'] = 0

position = 0
trade_times = 0
data_zscore['costbased_close'] = data_zscore['close']  # 总交易费用
...
for i in range(0, data_zscore['z_score'].shape[0]):
    ...
    if (position == 0) and zscore > buy_thre:
        ...
        data_zscore.loc[i, 'costbased_close'] = data_zscore.loc[i, 'costbased_close'] * (1 + transaction_cost_rate)
        # 考虑交易费用，在第i天把收盘价乘以一个费用，以表示成本价。
        position = 1
        trade_times += 1
    elif (position == 1) and (zscore < sell_thre):
        # 若之前有持仓，下穿卖出阈值则卖出
        data_zscore.loc[i, 'flag'] = -1
        data_zscore.loc[i + 1, 'position'] = 0
        data_zscore.loc[i, 'costbased_close'] = data_zscore.loc[i, 'costbased_close'] * (1 - transaction_cost_rate)
        position = 0
        trade_times += 1
    else:
...
data_zscore['cost_pct'] = data_zscore['costbased_close'] / data_zscore['costbased_close'].shift(1) - 1

data_zscore.dropna(inplace=True)
data_zscore['strategy_pct'] = data_zscore['cost_pct'] * data_zscore['position']
...
print('RSRS标准分策略的考虑交易成本后总收益率：%.2f%%，同期沪深300总收益率为:%.2f%%' % (total_ret, total_ret300))
print('RSRS标准分策略的考虑交易成本后年化收益率：%.2f%%，同期沪深300年化收益率为：%.2f%%' %(annual_ret,annual_ret300))
