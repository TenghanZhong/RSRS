##改进原理：根据Z_score和未来十日期望收益率和上涨概率的关系图发现，他们的关系有有偏倾向
#Z值越高，未来十日期望收益率越大（两者相关系数高）
#因而考虑，通过使z值的值变大，提高超额收益，而又观察到斜率均值为0.9，因而可以直接乘斜率得到右边zscore

###要根据斜率分布的均值标准差判断，斜率均值是0.9，因此对于小于0.9的斜率，标准化后得到标准分小于0，此时在标准分基础上乘斜率就会使标准分右偏。
#对于0.9-1区间内的值，标准分乘斜率会左偏一些
#对大于1的值，标准分还是右偏。
#因而总体来说，乘斜率可以达到右偏的效果。
...
#002594.XSHE比亚迪
ts_code='000300.SH'
data_zscore = pro.index_daily(ts_code=ts_code, start_date='20050101', end_date='20170301',fields=[
        "trade_date",
        "close",
        "high",
        "low",
...
def cal_beta(df, rolling):
    if df.shape[0] < rolling:
        return np.nan
    y = df['high'].values
    x = df['low'].values.reshape(-1, 1)#reshape是便于拟合
...
from backtest import run_Timing_Strategy
# 运行回测
results = run_Timing_Strategy(data_zscore,ts_code)
# 打印或处理结果
print(results)
...
labels = np.arange(-5,4,0.2)+ 0.1  # 区间标签,加0.1是为了将标签置于区间中央，0.5*D
